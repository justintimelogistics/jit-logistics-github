...
function buildCookieAttributes(hostname, maxAgeSeconds) {
  const parts = [
    "Path=/",
    "HttpOnly",
    "Secure",
  ...
  if (hostname && hostname !== "localhost" && !hostname.endsWith(".pages.dev")) {
...
export async function createSessionCookie(secret, hostname) {
  ...
  return `${SESSION_COOKIE}=${token}; ${buildCookieAttributes(hostname, SESSION_MAX_AGE_SECONDS)}`;
}
...
export function clearSessionCookie(hostname) {
  return `${SESSION_COOKIE}=; ${buildCookieAttributes(hostname, 0)}`;
}
