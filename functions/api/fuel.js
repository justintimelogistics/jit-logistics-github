...
export async function onRequestPut(context) {
  ...
  const gallons = Number(body?.gallons);
  const pricePerGallon = Number(body?.pricePerGallon);
  const createdAt = body?.createdAt ? String(body.createdAt).trim() : new Date().toISOString();
  ...
  const ordered = [...(otherEntriesResult.results ?? []), { id, odometer, created_at: createdAt }]
  ...
  await context.env.DB.prepare(`
    UPDATE fuel_entries
    SET created_at = ?2, odometer = ?3, gallons = ?4, price_per_gallon = ?5, total_cost = ?6, mpg = NULL
    WHERE id = ?1
  `)
    .bind(id, createdAt, odometer, gallons, pricePerGallon, gallons * pricePerGallon)
    .run();
...
export async function onRequestDelete(context) {
  ...
  if (!Number.isFinite(id)) {
    return json({ error: "Fuel id is required." }, { status: 400 });
  }

  await context.env.DB.prepare(`DELETE FROM fuel_entries WHERE id = ?1`).bind(id).run();
  await normalizeEntries(context.env);
  const entries = await listEntries(context.env);
  return json({ entries });
}
