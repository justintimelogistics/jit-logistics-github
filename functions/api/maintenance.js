...
export async function onRequestPut(context) {
  ...
  await context.env.DB.prepare(`
    ...
    SET service_date = ?2, odometer = ?3, service_type = ?4, cost = ?5, next_due_mileage = ?6, next_due_date = ?7, notes = ?8
    WHERE id = ?1
  `)
    .bind(id, serviceDate, odometer, serviceType, cost, nextDueMileage, nextDueDate, notes)
    .run();
...
export async function onRequestDelete(context) {
  ...
  await context.env.DB.prepare(`DELETE FROM maintenance_entries WHERE id = ?1`).bind(id).run();
  const entries = await listEntries(context.env);
  return json({ entries });
}
