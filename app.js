...
function getLatestOdometer() {
  return fuelEntries.length > 0 ? Number(fuelEntries[0].odometer) : null;
}
...
function renderOverviewSection() {
  ...
  overviewMpgMeta.textContent = overviewHoldState.mpg ? "YTD MPG from January 1" : "Rolling 30-day MPG";
  ...
  overviewFuelPriceMeta.textContent = overviewHoldState.fuelPrice ? "YTD average price per gallon" : "Rolling 30-day average price per gallon";

  const latestOdometer = getLatestOdometer();
  const pmCountdown = getMilesToNextPm(latestOdometer);
  ...
  overviewPmMeta.textContent = overviewHoldState.pm ? "YTD miles from January 1" : "Miles remaining to the next 10,000-mile PM";

  const monthCostPerMile = getCostPerMile(monthEntries);
  const ytdCostPerMile = getCostPerMile(ytdEntries);
  ...
  overviewCostMonthMeta.textContent = overviewHoldState.costMonth ? "YTD cost per mile from January 1" : "Month-to-date cost per mile";

  const rollingCostPerMile = getCostPerMile(rollingThirtyEntries);
  const closedThreeMonthCostPerMile = getCostPerMile(closedThreeMonthEntries);
  ...
  overviewCostRollingMeta.textContent = overviewHoldState.costRolling ? "Cost per mile for the last 3 closed months" : "Rolling 30-day cost per mile";
}
