...
const ctHutRangeButtons = Array.from(document.querySelectorAll("[data-cthut-range]"));
...
function setCtHutRange(range) {
  ctHutRange = range;
  ctHutRangeButtons.forEach((button) => {
    button.classList.toggle("report-filter-active", button.dataset.cthutRange === range);
  });
  renderCtHutReport();
}
...
function refreshDashboardClocks() {
  ...
  setGaugeState(weeklyGauge, weeklyRatio, getBlueYellowRedColor(weeklyRatio));
...
ctHutRangeButtons.forEach((button) => {
  button.addEventListener("click", () => setCtHutRange(button.dataset.cthutRange));
});
...
ctHutButton.addEventListener("click", openCtHutModal);
managementCtHutButton.addEventListener("click", openCtHutModal);
