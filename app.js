...
const todaySummaryValue = document.getElementById("todaySummaryValue");
const todaySummaryCopy = document.getElementById("todaySummaryCopy");
const tomorrowSummaryValue = document.getElementById("tomorrowSummaryValue");
const tomorrowSummaryCopy = document.getElementById("tomorrowSummaryCopy");
const resetSummaryValue = document.getElementById("resetSummaryValue");
const resetSummaryCopy = document.getElementById("resetSummaryCopy");
...
function refreshDashboardClocks() {
  ...
  if (state.onDuty) {
    todaySummaryValue.textContent = `${formatDuration(state.dutyRemainingMs)} left today`;
    todaySummaryCopy.textContent = `Weekly availability right now: ${formatDuration(state.weeklyRemainingMs)}.`;
  } else if (state.breakMet) {
    todaySummaryValue.textContent = "Fresh start ready";
    todaySummaryCopy.textContent = "Your next punch in can start a fresh 14-hour window.";
  } else if (state.lastOut) {
    todaySummaryValue.textContent = `${formatDuration(Math.max(0, 10 * HOUR_MS - state.offDutyMs))} break still needed`;
    todaySummaryCopy.textContent = "You still owe off-duty time before the 10-hour break is fully met.";
  } else {
    todaySummaryValue.textContent = "Waiting to clock in";
    todaySummaryCopy.textContent = "Your current duty picture will show here.";
  }

  tomorrowSummaryValue.textContent = `${formatDuration(state.tomorrowWeeklyRemainingMs)} tomorrow`;
  tomorrowSummaryCopy.textContent = state.tomorrowRecapGainMs > 0
    ? `${formatDuration(state.tomorrowRecapGainMs)} comes back at midnight from recap.`
    : "No new recap hours are due back at midnight.";

  if (state.breakMet) {
    resetSummaryValue.textContent = "10-hour reset met";
    resetSummaryCopy.textContent = "You are clear to start a fresh duty window.";
  } else if (!state.onDuty && state.lastOut) {
    resetSummaryValue.textContent = `${formatDuration(Math.max(0, 34 * HOUR_MS - state.offDutyMs))} to 34`;
    resetSummaryCopy.textContent = state.offDutyMs >= 34 * HOUR_MS
      ? "34-hour reset complete."
      : "If you stay off duty, this is what remains for a full 34-hour reset.";
  } else {
    resetSummaryValue.textContent = "Not started";
    resetSummaryCopy.textContent = "A 34-hour reset begins after you punch out.";
  }
